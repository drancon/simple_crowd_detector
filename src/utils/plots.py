...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=True):
        ...
        self.filter_boxes, self.filter_names, self.filter_scores = [], [], []
    ...
    def blur_box_label(self, box, name='', prob='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        # Add one xyxy box to image with label
        # cv2
        if name in ['person'] and prob > 0.6:
            c1, c2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
            cv2.rectangle(self.im, c1, c2, color, thickness=self.lw, lineType=cv2.LINE_AA)
            
            # crop and blur body
            body = self.im[c1[1]:c2[1], c1[0]:c2[0], :]
            self.im[c1[1]:c2[1], c1[0]:c2[0], :] = cv2.resize(
                cv2.GaussianBlur(cv2.resize(body, (20,8)), (5,5), cv2.BORDER_DEFAULT),
                (c2[0]-c1[0], c2[1]-c1[1]))
            
            self.filter_boxes.append(box)
            self.filter_names.append(name)
            self.filter_scores.append(prob)
        elif name in ['car', 'bus', 'truck'] and prob > 0.6:
            c1, c2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
            cv2.rectangle(self.im, c1, c2, color, thickness=self.lw, lineType=cv2.LINE_AA)
            self.filter_boxes.append(box)
            self.filter_names.append(name)
            self.filter_scores.append(prob)
        elif name in ['bench', 'chair', 'umbrella']:
            c1, c2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
            cv2.rectangle(self.im, c1, c2, color, thickness=self.lw, lineType=cv2.LINE_AA)
            self.filter_boxes.append(box)
            self.filter_names.append(name)
        ...
        tf = max(self.lw - 1, 1)  # font thickness
        w, h = cv2.getTextSize(label, 0, fontScale=self.lw / 4, thickness=tf)[0]
        c2 = c1[0] + w, c1[1] - h - 3
        cv2.rectangle(self.im, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(self.im, label, (c1[0], c1[1] - 2), 0, self.lw / 3, txt_color, thickness=tf,
                    lineType=cv2.LINE_AA)
    ...
    def get_detection(self):
        return np.asarray(self.filter_boxes), self.filter_names, np.asarray(self.filter_scores)
